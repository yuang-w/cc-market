---
name: autogdb-investigation
description: Use when investigating coredumps, crashes, performance issues, or other anomalies on production machines using autogdb MCP tools
---

# autogdb Investigation

## Overview

Root cause investigation skill for production debugging with autogdb MCP. Supports coredumps, performance issues, and anomalies where source code is typically unavailable.
...
| **Socket** | User already has GDB running and listening on domain socket | Socket path (user ran `autogdb listen`) |
...
**Note:** In **socket** mode, the inferior's stdout and stderr are not captured through the autogdb/MCP connection. Redirect program output to files when starting it (e.g. when using `run`), then read those files.
