...
def _install_auto_gdb_command():
    ...
    class AutoGdb(gdb.Command):
        """GDB ``auto-gdb`` command: ``listen`` / ``stop`` are parsed from the argument string.
        ...
        * ``--echo`` — mirror MCP command CLI output to this GDB terminal.
        ...
        def serve_loop(self, sock):
            ...
            while not self.stop_event.is_set():
                try:
                    ...
                    gdb.write("auto-gdb: accept failed: {}\n".format(e), gdb.STDERR)
                    break
        ...
        def invoke(self, arg, from_tty):
            ...
            if sub == "listen":
                ...
                gdb.write(
                    "auto-gdb: unknown subcommand {!r} (expected listen or stop)\n".format(parts[0]),
                    gdb.STDERR,
                )
                self._write_command_doc()
        ...
        def _invoke_listen(self, parts):
            ...
            if self.thread is not None and self.thread.is_alive():
                gdb.write(
                    "auto-gdb listen: already listening; run auto-gdb stop first\n",
                    gdb.STDERR,
                )
                return
            ...
            try:
                ...
                gdb.write(
                    "auto-gdb listen: could not remove existing socket: {}\n".format(e), gdb.STDERR
                )
                return
            ...
            try:
                ...
                gdb.write("auto-gdb listen: bind failed: {}\n".format(e), gdb.STDERR)
                try:
                    sock.close()
                except OSError:
                    pass
            ...
            gdb.write("auto-gdb listen: listening on {} ({})\n".format(path, mode), gdb.STDOUT)

        def _invoke_stop(self, parts):
            if parts:
                gdb.write("auto-gdb stop: unexpected arguments\n", gdb.STDERR)
                self._write_command_doc()
                return

            if self.thread is None or not self.thread.is_alive():
                gdb.write("auto-gdb stop: not listening\n", gdb.STDERR)
                return
            ...
            if self.server_sock is not None:
                try:
                    ...
                    gdb.write(
                        "auto-gdb stop: close server socket failed: {}\n".format(e), gdb.STDERR
                    )
            self.thread.join()
            self.thread = None
            self.server_sock = None
