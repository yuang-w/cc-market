...
import (
	...
	"github.com/yuang-w/cc-market/plugins/auto-gdb/src/internal/gdb"
)

// Server instructions for MCP clients.
const serverInstructions = `MCP server that wraps GDB over the CLI. One GDB session per server process. Call create_session (empty socket_path spawns a local GDB subprocess; or set socket_path to a Unix socket where external GDB runs auto-gdb-listen after loading gdb_bridge.py). Then drive GDB only via gdb_command (e.g. file /path/to/binary, break main, run, bt, info registers). Use session_status / stop_session as needed. All tools return a JSON object (dict).`
...
func createSessionHandler(ctx context.Context, req *mcp.CallToolRequest, args createSessionInput) (*mcp.CallToolResult, createSessionOutput, error) {
	...
	if socketPath != "" {
		...
		if err != nil {
			controller.Exit()
			controller = nil
			return nil, createSessionOutput{Error: fmt.Sprintf("timeout or error on show version (is auto-gdb-listen running?): %s", err)}, nil
		}
